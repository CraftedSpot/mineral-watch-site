...
// .wrangler/tmp/bundle-PKh0lS/checked-fetch.js
...
var USERS_TABLE = "\u{1F464} Users";
var PROPERTIES_TABLE = "\u{1F4CD} Client Properties";
var WELLS_TABLE = "\u{1F6E2}\uFE0F Client Wells";
var ACTIVITY_TABLE = "\u{1F4CB} Activity Log";
...
var ACTIVITY_LIMITS = {
  ...
  "Standard": 90,
  "Professional": 365 * 10,
  // 10 years = essentially unlimited
  "Enterprise": 365 * 10
};
var OCC_CACHE_TTL = 86400;
var MAX_NOTES_LENGTH = 1e3;
var CORS_HEADERS = {
...
var PRICE_IDS = {
  starter_monthly: "price_1SXDS9641UqM8A7NeSc0MOTv",
  // $9/mo
  starter_annual: "price_1SXDSG641UqM8A7N9PbCLsvG",
  // $86/yr
  standard_monthly: "price_1SXDSL641UqM8A7NSS10CWBd",
  // $29/mo
  standard_annual: "price_1SXDSQ641UqM8A7NIWcuCzmp",
  // $278/yr
  professional_monthly: "price_1SXDSV641UqM8A7NZTXdvUls",
  // $99/mo
  professional_annual: "price_1SXDSZ641UqM8A7NvTHEJy9s"
  // $950/yr
};
var PRICE_TO_PLAN = {
...
import dashboardHtml from "./fefae59c19c8480e2f1902dd343ae141451f7ce0-dashboard.html";
import loginHtml from "./e6f2ced2a3d85dd388ec4655d2af4a2076866368-login.html";
import accountHtml from "./61f3ec7a3ec5e9f0cdf6d23b2f7ff1a07742d66d-account.html";
import upgradeHtml from "./d50d8202f262c3bf4b534212639e5c003756865f-upgrade.html";
...
__name(fetchUserProperties, "fetchUserProperties");
...
__name(fetchUserWells, "fetchUserWells");
...
__name(getFreeWelcomeEmailHtml, "getFreeWelcomeEmailHtml");
...
__name(getFreeWelcomeEmailText, "getFreeWelcomeEmailText");
...
async function fetchWellDetailsFromOCC(apiNumber, env) {
  ...
  try {
    ...
    if (data.features && data.features.length > 0) {
      ...
      const wellDetails = {
        api: attr.api,
        wellName: attr.well_name && attr.well_num && !attr.well_name.includes("#") ? `${attr.well_name} #${attr.well_num}` : attr.well_name || "",
...
__name(normalizeSectionNumber, "normalizeSectionNumber");
...
__name(normalizeTownship, "normalizeTownship");
...
__name(normalizeMeridian, "normalizeMeridian");
function normalizeCounty(value) {
  if (!value) return "";
  const str = String(value).trim();
  return str.split(" ").map((word) => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(" ");
}
__name(normalizeCounty, "normalizeCounty");
...
__name(normalizePropertyData, "normalizePropertyData");
function validateTownship(value) {
  if (!value) return false;
  return /^\d+[NS]$/i.test(value);
}
__name(validateTownship, "validateTownship");
...
async function handleBulkValidateProperties(request, env) {
  ...
  const existingSet = new Set(
    existingProperties.map(
      (p) => `${p.SEC}-${p.TWN}-${p.RNG}-${p.MERIDIAN || "IM"}`
    )
  );
  const userRecord = await getUserById(env, user.id);
  const plan = userRecord?.fields.Plan || "Free";
...
__name(handleBulkValidateProperties, "handleBulkValidateProperties");
async function handleBulkUploadProperties(request, env) {
  ...
  const toCreate = properties.filter((prop) => {
    ...
    return !existingSet.has(key) && prop.SEC >= 1 && prop.SEC <= 36 && validateTownship(prop.TWN) && validateRange(prop.RNG);
  });
...
__name(handleBulkUploadProperties, "handleBulkUploadProperties");
...
__name(handleBulkValidateWells, "handleBulkValidateWells");
async function handleBulkUploadWells(request, env) {
  ...
  const toCreate = wells.filter((well) => {
    return !existingSet.has(well.apiNumber) && well.apiNumber.length === 10 && well.apiNumber.startsWith("35");
  });
  const results = {
    successful: 0,
    failed: 0,
  ...
  function generateMapLink3(lat, lon, title) {
    if (!lat || !lon) return "#";
    const appId = "ba9b8612132f4106be6e3553dc0b827b";
    const markerTemplate = JSON.stringify({
      title: title || "Well Location",
  ...
  __name(generateMapLink3, "generateMapLink");
  const batchSize = 10;
    ...
    const response = await fetch(`https://api.airtable.com/v0/${BASE_ID}/${encodeURIComponent(WELLS_TABLE)}`, {
      ...
      body: JSON.stringify({
        records: batch.map((well) => {
          ...
          const mapLink = occ.lat && occ.lon ? generateMapLink3(occ.lat, occ.lon, occ.wellName) : "#";
          return {
            fields: {
              User: [user.id],
...
__name(handleBulkUploadWells, "handleBulkUploadWells");

// src/handlers/track-well.ts
function generateMapLink2(lat, lon, title) {
  if (!lat || !lon) return "#";
  const appId = "ba9b8612132f4106be6e3553dc0b827b";
  const markerTemplate = JSON.stringify({
    title: title || "Well Location",
...
__name(generateMapLink2, "generateMapLink");
...
__name(validateTrackToken, "validateTrackToken");
...
<body>
    <div class="container">
        <div class="card">
            <div class="success-icon">${alreadyTracking ? "\u2713" : "\u{1F389}"}</div>
            <h1>${alreadyTracking ? "Already Tracking" : "Successfully Added!"}</h1>
            <p class="subtitle">
                ${alreadyTracking ? "You're already tracking this well." : "This well has been added to your tracking list."}
            </p>
...
__name(generateTrackWellSuccessPage, "generateTrackWellSuccessPage");
...
<body>
    <div class="container">
        <div class="card">
            ...
            <div class="buttons">
                ${showUpgrade ? '<a href="' + BASE_URL + '/portal/upgrade" class="btn btn-warning">Upgrade Plan</a>' : ""}
...
__name(generateTrackWellErrorPage, "generateTrackWellErrorPage");
async function handleTrackThisWell(request, env, url) {
  ...
  if (userId && token && exp && env.TRACK_WELL_SECRET) {
    ...
    try {
      ...
      let wellStatus = "";
      if (wellDetails) {
        occMapLink = generateMapLink2(wellDetails.lat, wellDetails.lon, wellDetails.wellName);
...
// .wrangler/tmp/bundle-PKh0lS/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-PKh0lS/middleware-loader.entry.ts
