...
export function createStatewideActivityFromPermit(permit, wellCoords = null, mapLink = null) {
  const activityData = {
    ...
    county: (permit.County || '').indexOf('-') > 0 ? permit.County.substring(permit.County.indexOf('-') + 1) : (permit.County || ''),
...
export function createStatewideActivityFromCompletion(completion, wellCoords = null, mapLink = null) {
  const activityData = {
    ...
    county: (completion.County || '').indexOf('-') > 0 ? completion.County.substring(completion.County.indexOf('-') + 1) : (completion.County || ''),
