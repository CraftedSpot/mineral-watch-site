...
RENTAL DEPOSIT RECEIPT DETECTION:
- rental_deposit_receipt: Look for "RENTAL DEPOSIT", "DELAY RENTAL", "RENTAL PAYMENT", bank deposit references for lease rental payments. May appear as a letter format (cover letter enclosing a rental deposit) — if the substance is about depositing money at a bank for delay rental on a lease, classify as rental_deposit_receipt, NOT correspondence. Key indicators: bank name, deposit amount, lease period covered, lessor/lessee names, "for the credit of" language. These prove the lessee paid delay rentals to keep a lease alive during primary term.
